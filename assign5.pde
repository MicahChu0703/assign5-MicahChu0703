...
void draw() {
	...
	switch (gameState) {
		...
		for(int i = 0; i < cabbageX.length; i++){
			...
			if(playerHealth < PLAYER_MAX_HEALTH&&isHit(playerX,playerY,SOIL_SIZE,SOIL_SIZE,cabbageX[i],cabbageY[i],SOIL_SIZE,SOIL_SIZE)) { 
				playerHealth ++;
...
String convertFramesToTimeString(int frames){	// Requirement #4
  ...
  float ss=floor((frames/60))%60;
	return (nf(mm,2,0)+":"+nf(ss,2,0));
}

color getTimeTextColor(int frames){				// Requirement #5
  if(frames>=2*60*60){return #00ffff;}
  if(2*60*60>frames&&frames>=60*60){return #ffffff;}
  if(1*60*60>frames&&frames>=30*60){return #ffcc00;} 
  if(30*60>frames&&frames>=10*60){return #ff6600;} 
  if(10*60>frames){return #ff0000;}   
	return #ffffff;
}
